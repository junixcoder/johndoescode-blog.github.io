    ...
    await Promise.all(posts.map(async post => {
      if(!post.image){
        try{
          ...
          post.image = m ? m[1].replace('..','.') : 'images/placeholder.jpg';
        }catch(e){ post.image='images/placeholder.jpg'; }
      }
...
function renderCategories(){
  ...
  allBtn.textContent='All'; allBtn.classList.add('active');
  allBtn.onclick = ()=>{ filtered = posts.slice(); currentPage=1; updateActiveCategory(null); render(); };
  container.appendChild(allBtn);

  Array.from(categories).sort().forEach(cat=>{
    const btn = document.createElement('button');
    btn.textContent=cat;
    btn.onclick = ()=>{ filtered = posts.filter(p=> (p.categories||[]).includes(cat)); currentPage=1; updateActiveCategory(cat); render(); };
...
function updateActiveCategory(cat){
  ...
  buttons.forEach(b=>{ if(b.textContent===cat || (cat===null && b.textContent==='All')) b.classList.add('active'); else b.classList.remove('active'); });
}
...
document.getElementById('search').addEventListener('input', (e)=>{
  ...
  filtered = posts.filter(p=> p.title.toLowerCase().includes(q) || (p.description||'').toLowerCase().includes(q) );
